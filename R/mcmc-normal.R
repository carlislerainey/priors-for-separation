lp.normal <- function(beta, scale.coef, scale.int, X, y) {
  p <- plogis(matrix(X%*%beta))
  loglik <- sum(y*log(p)) + sum((1-y)*log(1 - p)) + 
    log(dcauchy(beta[1], scale = scale.int)) + 
    sum(log(dnorm(beta[2:length(beta)], sd = scale.coef)))
  return(loglik)
}

normal <- function(f, data, scale.coef = 2.5,
                     scale.int = 10, 
  ...
  mcmc <- MCMCmetrop1R(fun = lp.normal, theta.init = init, 
                       scale.int = scale.int,
...
# d <- data.frame(x1, y)
# m1 <- normal(y ~ x1, d, mcmc = 100, verbose = 10)
# plot(m1)
