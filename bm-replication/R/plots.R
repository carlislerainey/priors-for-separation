...
# risk-ratio plot 
pdf("doc/figs/bm-rr.pdf", height = 3.5, width = 7.7)
par(mfrow = c(1,1), mar = c(4,1,1,1), oma = c(0,0,0,0))
eplot(xlim = c(0.01, 10000000), ylim = c(1.75, 6.5),
      anny = FALSE,
...
text(s[1], 5.85, round(s[1], 2), cex = .5)
text(s[2], 5.85, round(s[2], 1), cex = .5)
text(s[3], 5.85, round(s[3], 1), cex = .5)
s <- sumry(skep.rr.sims, 5, "Skeptical Normal(0, 2) Prior")
text(s[1], 4.85, round(s[1], 2), cex = .5)
text(s[2], 4.85, round(s[2], 1), cex = .5)
text(s[3], 4.85, round(s[3], 1), cex = .5)
s <- sumry(enth.rr.sims, 4, "Enthusiastic Normal(0, 8) Prior")
text(s[1], 3.85, round(s[1], 2), cex = .5)
text(s[2], 3.85, round(s[2], 1), cex = .5)
text(s[3], 3.85, round(s[3], 1), cex = .5)
s <- sumry(zorn.rr.sims, 3, "Zorn's Default Jefferys' Prior")
text(s[1], 2.85, round(s[1], 2), cex = .5)
text(s[2], 2.85, round(s[2], 1), cex = .5)
text(s[3], 2.85, round(s[3], 1), cex = .5)
s <- sumry(gelman.rr.sims, 2, "Gelman et al.'s Default Cauchy(0, 2.5) Prior")
text(s[1], 1.85, round(s[1], 2), cex = .5)
text(s[2], 1.85, round(s[2], 1), cex = .5)
text(s[3], 1.85, round(s[3], 1), cex = .5)
dev.off()
